...
export function modelSupportsVision(): boolean {
  if (config.primaryModelProvider === 'deepseek') return false;
  return VISION_CAPABLE_PREFIXES.some(prefix => config.primaryModel.startsWith(prefix));
}

/** Get the primary language model, routing by provider */
export function getPrimaryModel(): LanguageModel {
  if (config.primaryModelProvider === 'deepseek') {
    if (!config.deepseekApiKey) {
      throw new Error('DEEPSEEK_API_KEY is required when PRIMARY_MODEL_PROVIDER=deepseek');
    }
    const deepseek = createOpenAI({
      apiKey: config.deepseekApiKey,
      baseURL: config.deepseekBaseUrl,
    });
    return deepseek.chat(config.deepseekModel);
  }
