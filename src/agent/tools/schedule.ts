...
export function scheduleTools(
  ...
  return {
    schedule_reminder: tool({
      description: 'Запланировать одноразовое напоминание пользователю через указанное количество минут. Используй когда пользователь просит: "напиши мне через минуту", "через 5 минут напомни", "засеки 10 минут", "через час напомни". Это НАПОМИНАНИЕ ПО ЗАПРОСУ ПОЛЬЗОВАТЕЛЯ — не путай с followup_ask.',
...
- Дни недели: 0=вс, 1=пн, 2=вт, 3=ср, 4=чт, 5=пт, 6=сб`,
    ...
    schedule_postpone_today: tool({
      ...
      execute: async ({ schedulerId, newTime }) => {
        ...
        // Compute delay in ms from now to newTime in user's timezone
        const now = Date.now();
        const tz = userTimezone || 'Asia/Almaty';
        ...
        // Skip today's original execution
        await jobSkipOnceRepo.set(schedulerId);

        // Create one-time reminder at new time
        const payload: JobPayload = {
          userId,
          telegramUserId,
          telegramChatId: chatId,
          kind: 'custom_reminder',
          context: `Отложенное напоминание (было ${job.kind}): ${job.payload?.context ?? 'напоминание'}`,
        };
